...
| 7 Per-minute win model | ✅ pipeline + experimental curve (2026-07-20) | `app/ml/` (DATASET_VERSION 2, MODEL_VERSION 1): each minute snapshot predicts the final blue-side result using only information available at that minute. Ranked solo reviews expose the model curve as experimental; unsupported queues use the rule-based fallback. The latest 526-match gate verdict remains `keep_heuristic` by Brier score. |
