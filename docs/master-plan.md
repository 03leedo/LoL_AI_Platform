...
### 7.4 확장 트랙 개편 — C1~C3 (2026-07-08, [PRD-expansion-2.md](./PRD-expansion-2.md) 수용)

기존 E1~E3 명명을 **컴패니언(C) 트랙**으로 재편한다. 쉬운 것부터 가치가 나오도록 분할:

- **C1 라이브 수집기** (난이도 하): 게임 감지(LCU) → Live Client Data 1초 폴링(내 체력·골드·스탯·이벤트)
  → 로컬 버퍼(SQLite) → 경기 후 서버 업로드 → ingest 파이프라인에서 Riot 데이터와 병합.
  LCU 픽창/큐 데이터 수집 포함 → **틸트 지표·체력 기반 도박사 정밀화**. 수집 전용, 경기 중 노출 없음.
- **C2 리플레이 분석기** (난이도 상, 핵심 해자): .rofl 다운로드 → Replay API 재생 제어 → 미니맵 CV
  → 초단위 10인 좌표 + fog 실측(회피 가능 데스) + 와드 좌표 지도. (구 E1)
- **C3 클립·쇼케이스**: Moment 기반 클라 자체 녹화 → **대표 하이라이트 프로필**(검색 결과에 장면 2~3개 노출,
  본인 등록 opt-in). (구 E2 확장)
- **리플레이 시어터** (구 "중계 대시보드" 재해석): 끝난 경기를 승률 곡선·변곡점 자막과 함께 중계 화면처럼
  재생. 프로 실시간 데이터 라이선스(Bayes 영역)·관전 3분 지연·실시간 노출 금지 원칙과 충돌 없이
  같은 기술 스택을 시연. 프로 대회 실시간 중계 도구는 보류.
- 라이브 이벤트 기반 자동 하이라이트(구 E3)는 C1+C3에 흡수.

**하이라이트 호스팅 하이브리드**:
1. 기본 — **YouTube 임베드**: 사용자 본인 채널에 OAuth 업로드 → 페이지에 iframe 재생. 스토리지 비용 0.
   제약: YouTube API 업로드 쿼터(기본 일 ~6개), API 업로드 영상은 앱 심사 전 비공개 잠금 정책 → 초기엔
   사용자 채널 업로드 방식으로 우회, 규모 확대 시 쿼터 증설 신청.
2. 확장 — **Cloudflare R2 셀프호스팅**: 15~20초 클립 2~3개/유저 제한, egress 무료. 제어·규모용.
...
## 11. M2 체크리스트 — 완료 (2026-07-08)

- [x] League-V4 랭크 조회(by-puuid) + `summoners` 랭크 컬럼(마이그레이션 0003) + 프로필 랭크 배지
- [x] 다중 경기 백그라운드 수집 (`services/ingest.py`): ingest_jobs 큐 → asyncio 태스크(자체 DB 세션)가
      매치·타임라인 수집 + 지표 계산·영속화 (LLM 제외), 진행률 폴링 API
- [x] 공통 6능력치 평가표 (`services/scorecard.py`, PRD §6): challenges + 축적된 metric_scores 집계,
      scope=aggregate로 영속화
- [x] 포지션 적합도/추천 (`services/role_analyzer.py`, PRD §8): 표본 축소 보정(shrinkage),
      적합도 50 미만은 추천 제외, 주의 포지션 판정
- [x] 승률 변곡점 (`services/turning_points.py`): 곡선 급변 ±8%p 이상 TOP3 + 인접 주요 이벤트 매칭,
      리뷰 응답 `turning_points`
- [x] 프론트: 랭크 배지 · 랭크 분석 패널(수집 버튼→진행률 폴링→6능력치 바/포지션 카드) · 변곡점 섹션

## 12. M3 체크리스트 — 완료 (2026-07-08)

- [x] 반복 패턴 탐지 (`services/patterns.py`): 첫 데스 시간대 클러스터 · 데스 존 반복 · 데스→오브젝트 손실 연계 ·
      제압골 헌납 · 만성 지표 임계값(강점/약점) — 전부 규칙 기반, 관측된 match_id 목록을 근거로 동반
- [x] 데스 부검 요약: 데스/킬 · 제압골 · 오브젝트 연계 비율 · 평균 첫 데스 시점
- [x] LLM 프로바이더 계층 (`services/llm_provider.py`): OpenAI 호환 + Anthropic, 키 없으면 규칙 기반 폴백.
      **LLM은 서술만 담당** — 수치·패턴은 규칙 계산 결과를 그대로 사용(환각 차단)
- [x] 리포트 서비스 + 캐시 (`services/reports.py`, analysis_reports 테이블 / 마이그레이션 0004):
      캐시 키 = 리포트 버전 + METRIC_VERSION + window + 최신 match_id → 새 데이터 없으면 LLM 재호출 없음
- [x] `GET /summoner/{name}/{tag}/report` (+force 재생성)
- [x] 프론트: AI 종합 리포트 패널 (생성 버튼 → 요약/강점·약점/개선 제안/패턴 카드/부검 칩, 생성 방식·캐시 표기)

**다음: M4 — 벤치마크("나 vs 상위 티어" + 티어 격차 진단) 및 승률 모델 학습(XGBoost+SHAP), 이후 M5 프로덕션.**
